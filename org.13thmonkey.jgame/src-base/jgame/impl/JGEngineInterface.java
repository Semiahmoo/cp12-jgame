...
public interface JGEngineInterface {
	...
	public static final String JGameVersionString = "3.6.2";
	...
	/**
	 * Gets (scaled) image directly. Is usually not necessary. Returns null if image
	 * is a null image; throws error if image is not defined.
	 */
	public JGImage getImage(String imgname);

	/**
	 * Gets (non-scaled) image's physical size directly. The object returned may be
	 * a reference to an internal variable, do not change it!
	 */
	 ...
	 */
	/**
	 * Define new sprite/tile image from a file. If an image with this id is already
	 * defined, it is removed from any caches, so that the old image is really
	 * unloaded. This can be used to load large (background) images on demand,
	 * rather than have them all in memory. Note that the unloading does not work
	 * for images defined from image maps. Defining an image with the same name and
	 * filename twice does not cause the image to be reloaded, but keeps the old
	 * image.
	 *
	 * @param name        image id
	...
	/**
	 * Define new image by rotating an already loaded image. This method does not
	 * yet work for images defined from image maps! The destination image is always
	 * a square which is large enough to fit the source image at any angle. Its
	 * dimension is calculated as: max(width, height, 0.75*(width+height)). The
	 * source image is rendered to its center.
	 * <p>
	 * If an image with this id is already defined, it is removed from any caches,
	 * so that the old image is really unloaded. This can be used to load large
	 * (background) images on demand, rather than have them all in memory. Note that
	 * the unloading does not work for images defined from image maps.
	 * 
	 ...
	 * demand, rather than have them all in memory. Note that the unloading does not
	 * work for images defined from image maps.
	 *
	 * @param name        image id
	 * @param tilename    tile id (1-4 characters)
	 * @param collisionid cid to use for tile collision matching
	 * @param imgwidth    width of image in pixels
	 * @param imgheight   height of image in pixels
	 * @param imgdata     array of rgba pixel values
	 * @param imgdataofs  offset of start of image in imgdata
	 * @param linewidth   number of bytes of one image line
	 * @param top         collision bounding box dimensions
	 * @param left        collision bounding box dimensions
	 * @param width       collision bounding box dimensions
	 * @param height      collision bounding box dimensions
	...
	/**
	 * Define image map, a large image containing a number of smaller images to use
	 * for sprites or fonts. The images must be in a regularly spaced matrix. One
	 * may define multiple image maps with the same image but different matrix
	 * specs.
	 * 
	...
	/**
	 * Define new sprite/tile image from a file, with collision bounding box equal
	 * to the image's dimensions. If an image with this id is already defined, it is
	 * removed from any caches, so that the old image is really unloaded. This can
	 * be used to load large (background) images on demand, rather than have them
	 * all in memory. Note that the unloading does not work for images defined from
	 * image maps. Defining an image with the same name and filename twice does not
	 * cause the image to be reloaded, but keeps the old image.
	 *
	 * @param imgname     image id
	 * @param tilename    tile id (1-4 characters)
	 * @param collisionid cid to use for tile collision matching
	...
	/**
	 * Define new sprite/tile image from map, with collision bounding box equal to
	...
	/**
	 * Load a set of imagemap, image, animation, and audio clip definitions from a
	 * file. The file contains one image / imagemap / animation definition / audio
	 * clip on each line, with the fields separated by one or more tabs. Lines not
	 * matching the required number of fields are ignored. The fields have the same
	 * order as in defineImage, defineImageMap, defineAnimation, and
	 * defineAudioClip. For example:
	 * <p>
	...
	* &nbsp;&nbsp;&nbsp; gfx/myimage.gif &nbsp;&nbsp;&nbsp; -</code>
	 * <p>
	 * with the whitespace between the fields consisting of one or more tabs. The
	 * defineAnimation methods take an array of names as the second argument. This
	 * is represented in table format as the names separated by semicolon ';'
	 * characters. So:
	 * <P>
	...
	* </code>
	...
	/**
	 * Set image to display behind transparent tiles. Image size must be a multiple
	 * of the tile size. Passing null turns off background image; the background
	...
	/**
	 * Set image to display at a particular parallax scroll level. Only some
	 * platforms support parallax scrolling. On Android, a call to this method will
	 * activate parallax scrolling mode, which is less efficient that basic mode.
	 * Parallax mode is disabled again by setting all BG images to null. On other
	 * platforms, only the level 0 image is displayed, with its offset equal to the
	 * view offset. Level 0 corresponds to the top level; setBGImage(String) is
	 * equivalent to setBGImage(String,0,true,true). The level 0 image follows the
	 * view offset by default, higher levels are initialised to offset (0,0) by
	 * default.
	 * 
	...
	/**
	 * Add new object, will become active next frame, do not call directly. This
	 * method is normally called automatically by the JGObject constructor. You
	...
	/**
	 * Call the move() methods of those objects matching the given name prefix and
	 * collision id mask.
	 * 
	...
	/**
	 * Calls all colliders of objects that match dstcid that collide with objects
	 * that match srccid.
	 */
	public void checkCollision(int srccid, int dstcid);

	/**
	 * Checks collision of objects with given cid mask with given object. Suspended
	 * objects are not counted (same as checkCollision(int,int)). This method should
	 * be a more efficient way to check for object overlap than getObjects, though
	...
	/**
	 * Check collision of tiles within given rectangle, return the OR of all cids
	 * found.
	 * 
	...
	/**
	 * Calls all bg colliders of objects that match objid that collide with tiles
	 * that match tileid.
	 */
	public void checkBGCollision(int tilecid, int objcid);
	...
	/**
	 * Query the object list for objects matching the given name prefix, CID mask,
	 * and collide with the given bounding box. If suspended_obj is true, suspended
	 * objects are also included. The list of objects returned match all the
	 * supplied criteria. This is an inefficient method, use sparingly.
	 * 
	 * @param cidmask       collision id mask, 0 means ignore
	 * @param prefix        ID prefix, null means ignore
	 ...
	 * moveObjects or check*Collision ends, or immediately if done from within the
	 * main doFrame loop.
	 */
	public void removeObject(JGObject obj);

	/**
	 * Remove all objects which have the given name prefix and/or match the given
	 * cidmask. It also removes suspended objects. The actual matching and removal
	 * is done after the current moveObjects or check*Collision ends, or immediately
	 * if done from within the main doFrame loop. It also removes any matching
	...
	/**
	 * Remove all objects which have the given name prefix and/or match the given
	 * cidmask. You can specify whether to remove suspended objects or not. The
	 * actual matching and removal is done after the current moveObjects or
	 * check*Collision ends, or immediately if done from within the main doFrame
	 * loop. It also removes any matching objects which are pending to be added the
	 * next frame.
	 * 
	 * @param cidmask       collision id mask, 0 means ignore
	 * @param prefix        ID prefix, null means ignore
	...
	/**
	 * Count how many objects there are with both the given name prefix and have
	 * colid&amp;cidmask != 0. Either criterion can be left out. It also counts
	 * suspended objects. Actually searches the object array, so it may be
	 * inefficient to use it a lot of times.
	 * 
	 * @param cidmask collision id mask, 0 means ignore
	 * @param prefix  ID prefix, null means ignore
	 */
	...
	/**
	 * Count how many objects there are with both the given name prefix and have
	 * colid&amp;cidmask != 0. Either criterion can be left out. You can specify
	 * whether to count suspended objects or not. Actually searches the object
	 * array, so it may be inefficient to use it a lot of times.
	 * 
	 * @param cidmask       collision id mask, 0 means ignore
	 * @param prefix        ID prefix, null means ignore
	 * @param suspended_obj also count suspended objects
	...
	/**
	 * Count number of tiles with given mask. Actually searches all tiles, so it's
	 * inefficient and should be used sparingly (such as, determine the number of
	 * something at the beginning of a game).
	 */
	public int countTiles(int tilecidmask);

	/**
	 * Get collision id of tile at given tile index position. Moduloes the given
	 * position if wraparound
	 */
	public int getTileCid(int xidx, int yidx);

	/**
	 * get string id of tile at given index position. Moduloes the given position if
	 * wraparound
	 */
	 ...
	 * coordinates. Get tile position range of all tiles overlapping given
	 * rectangle. Returns null is rectangle is null.
	 * 
	...
	/**
	 * Get tile index range of all tiles overlapping given rectangle of pixel
	 * coordinates, version without object creation. Get tile position range of all
	 * tiles overlapping given rectangle. Returns false is rectangle is null.
	 * 
	 * @param r    rectangle in pixel coordinates, null is none
	...
	/**
	 * Draw tile directly on background, do not call this directly, use setTile
	 * instead.
	 */
	...
	/**
	 * Set a block of tiles according to the single-letter tile names in the nxm
	 * character array tilemap.
	 */
	public void setTiles(int xofs, int yofs, String[] tilemap);

	/**
	 * Set a block of tiles according to the tile names in the nxm element array
	 * tilemap. The tile names may be multiple characters. Each String in the
	 * tilemap consists of a list of tile names separated by spaces. So:
	 * <code> "x aa ab abc"</code> stands for a sequence of four tiles, "x", "aa",
	 * "ab", and "abc".
	 */
	...
	/**
	 * Get the tile string of the point that is (xofs,yofs) from the tile index
	 * coordinate center.
	 */
	public String getTileStr(JGPoint center, int xofs, int yofs);

	/**
	 * Convert tile name to integer ID code (as used internally). The ID code
	 ...
	 * That is, the returned x/ypos falls within -pfwidth/height_half and
	 * pfwidth/height_half of x/yofs_mid
	 */
	...
	/**
	 * Show bounding boxes around the objects: the image bounding box (getBBox) ,
	 * the tile span (getTiles), and the center tiles (getCenterTiles).
	 */
	public void dbgShowBoundingBox(boolean enabled);

	/**
	 * Show the game state in the bottom right corner of the screen. The message
	 * font and foreground colour are used to draw the text.
	 */
	public void dbgShowGameState(boolean enabled);

	/**
	 * Indicates whether to show full exception stack traces or just the first
	 * lines. Default is false.
	 */
	public void dbgShowFullStackTrace(boolean enabled);

	/**
	 * Output messages on playfield instead of console. Default is true. Messages
	 * printed by an object are displayed close to that object. Messages printed by
	 * the main program are shown at the bottom of the screen. The debug message
	 * font is used to display the messages.
	 * <p>
	 * A message that is generated in this frame is shown in the foreground colour
	 * at the appropriate source. If the source did not generate a message, the last
	 * printed message remains visible, and is shown in debug colour 1. If an object
	 * prints a message, and then dies, the message will remain for a period of time
	 * after the object is gone. These messages are shown in debug colour 2.
	 */
	public void dbgShowMessagesInPf(boolean enabled);

	/**
	 * Set the number of frames a debug message of a removed object should remain on
	 * the playfield.
	 */
	...
	/**
	 * Exit, optionally reporting an exit message. The exit message can be used to
	 * report fatal errors. In case of an application or midlet, the program exits.
	 * In case of an applet, destroy is called, and the exit message is displayed on
	 * the playfield.
	 * 
	...
	/**
	 * Init engine as component to be embedded in a frame or panel; call this in
	 * your engine constructor.
	 * 
	...
	/**
	 * Set canvas dimensions and message colours/fonts. You must call this in
	 * initCanvas().
	 * 
	...
	/**
	 * Set scaling preferences for translating the virtual playfield to the actual
	 * display. You can only call this in initCanvas(). You can set the allowed
	 * aspect ratio and the crop margin here. Aspect ratio is defined as the ratio
	 * (actual_tile_width / actual_tile_height) / (virtual_tile_width /
	 * virtual_tile_height). So, if the tile size of the scaled display is (3,2)
	 * pixels, and the original was (4,4) pixels, the aspect ratio is 1.5. Default
	 * values for min_aspect_ratio and max_aspect_ratio are resp 0.75 and 1.333.
	 * Setting both to 1.0 means you always get a square aspect ratio.
	 * <P>
	 * Crop margin can be used if you wish to allow the scaling algorithm to take
	 * just a few pixels off your playfield in order to make a wider tile size fit.
	 ...
	 * small screens. Cropping just a few pixels off the playfield may be just
	 * enough to make the tiles 1 pixel larger. Setting a crop to a value greater
	 * than zero means you allow the playfield to fall off the canvas for the amount
	 * of actual pixels specified, in order to make a larger tile size fit. Default
	 * crop margin is 0.
	 * <P>
	 *
	...
	/**
	 * Magnification can be set to smooth or blocky. For platforms that enable
	 * smooth magnification (OpenGL), smoothing may look too blurry when magnifying
	 * by a large amount, so blocky magnification may actually look more charming.
	 * 
	...
	/**
	 * Get view offset as it will be at the next frame draw, in case we are not
	 * inside a frame draw, or the view offset as it is, when we are.
	 */
	public int viewXOfs();
	 ...
	 * This method is meant for doing initialisations after the applet has been
	 * initialised (in case we're an applet) but before the engine initialises. This
	 * can be considered a replacement of the regular constructor, making it
	 * independent of whether we're an applet or application. Typically you only
	 * need to call setCanvasSettings here, and, optionally,
	 * setScalingPreferences(). This is the place where you can read applet
	 * parameters and initialise accordingly. In case you want to adapt to the real
	 * display dimensions, you can get them using displayWidth/Height at this point.
	 */
	abstract public void initCanvas();

	/**
	 * Override to define your own initialisations after the engine initialised.
	 * This method is called by the game thread after initEngine(),
	 * initEngineApplet(), or initEngineComponent() was called.
	 */
	...
	/**
	 * signal that the engine should stop running and wait. May be called by the web
	 * browser.
	 */
	public void stop();

	/**
	 * Called when midlet is first initialised, or unpaused. Midlet version of
	 * init() when called for the first time, and start() for subsequent calls.
	 */
	public void startApp();

	/**
	 * Called by the application manager to pause app. Basically the midlet version
	 * of stop(), behaves the same as stop().
	 */
	public void pauseApp();

	/**
	 * Called by the application manager to exit app. Basically the midlet version
	 * of destroy(), behaves the same as destroy().
	 */
	...
	/**
	 * Make engine call start() when a key is pressed. This can be used to determine
	 * a start criterion when halting the engine from within using stop().
	 * 
	...
	/**
	 * Destroy function for deinitialising the engine properly. This is called by
	 * the applet viewer to dispose the applet. Use exitEngine to destroy the applet
	 * and exit the system.
	 */
	public void destroy();
	...
	/**
	 * Change offset of playfield view. The offset will become active at the next
	 * frame draw. If the view would be out of the playfield's bounds, the offset is
	 * corrected so that it is inside them. The offset of the parallax level 0
	 * background image is set to the offset as well, the other levels remain
	 * unchanged.
	 * 
	...
	/**
	 * Change (absolute) offset of BG image independently of view offset. Only
	 * supported by parallax scrolling platforms. On Android, a call to this method
	 * will activate parallax scrolling mode, which is less efficient that basic
	 * mode. Parallax mode is disabled again by setting all BG images to null. Note
	 * that parallax level 0 follows the view offset, so a call to this method to
	 * set level 0 should be done after calling setViewOffset.
	 * 
	...
	/**
	 * Zoom/rotate view. Can be used to create special effects, like speed-dependent
	 * zoom, explosion shake, etc. Only works in OpenGL. If you zoom out too far,
	 * parts that are beyond the borders of the defined view may get exposed, the
	 * appearance of which is undefined. This also happens when you rotate, so you
	 * will also need to zoom in to ensure this does not happen. Everything that is
	 * drawn relative to the playfield is zoomed/rotated, everything that is not is
	 * unaffected. Game logic is unaffected.
	 * <P>
	 * Mouse coordinates are inverse projected through the last set zoom/rotate
	 * setting, so that a playfield relative pixel drawn at the logical mouse
	 * coordinates coincides with the physical position of the mouse pointer. If you
	 * don't want this, you can set zoom/rotate to (1,0), read the mouse position,
	 * then set zoom/rotate to the desired value.
	 * <P>
	 * The zoom/rotate setting used for actual drawing is the last value set at the
	 * end of the doFrame phase. Should not be called during the paintFrame phase.
	 * 
	...
	/**
	 * Set playfield wraparound setting. When wraparound is enabled, the playfield
	 * theoretically behaves as if it is infinitely long or high, with tiles and
	 * objects repeating periodically, with playfield size being the period. Tile
	 * coordinates in a wrapped playfield are effectively modulo the playfield size.
	 * Object coordinates are wrapped symmetrically to the view offset, that is,
	 * they are kept within -playfieldsize, +playfieldsize of the center of the view
	 * offset. This ensures that regular coordinate comparison and collision usually
	 * work as expected, without having to actually model the infinite repetition of
	 * the objects in the wrap direction. The shiftx and shifty can be used to shift
	 * the object's wrap center by some pixels, to ensure they enter and leave the
	 * sides / top-bottom of the screen neatly when the playfield is not larger than
	 * the view. For this case, use the following formula: ensure the playfield is
	 * slightly larger than the view (namely, one sprite length), and set the shift
	 * to sprite length/2.
	 */
	public void setPFWrap(boolean wrapx, boolean wrapy, int shiftx, int shifty);

	/**
	 * Set frame rate in frames per second, and maximum number of frames that may be
	 * skipped before displaying a frame again. Default is 35 frames per second,
	 * with a maxframeskip of 4.
	 * 
	...
	/**
	 * Enable/disable video synced update (jogl only). This method has no effect on
	 * non-jogl platforms, where it is always disabled. The game state update
	 * becomes synced with the screen refresh rate. Frame rate is no longer fixed,
	 * but depends on the machine the game is running on. Gamespeed is set at the
	 * beginning of each frame to compensate for this. Gamespeed is 1 when actual
	 * frame rate == frame rate set with setFrameRate, less than 1 if frame rate
	 * greater than setFrameRate, more than 1 if frame rate less than setFrameRate.
	 * There is a hard upper and lower bound for gamespeed, to ensure it does not
	 * attain wild values under rare conditions. Lower bound for game speed is
	 * determined by a fixed upper bound for the expected screen refresh rate, 95
	 * hz. Upper bound for game speed is determined by the frameskip setting.
	 */
	public void setVideoSyncedUpdate(boolean value);

	/**
	 * Set game speed variable, default is 1.0. Game speed affects certain parts of
	 * the game engine automatically to offload some of the work involved of
	 * adapting a game to different speeds. These are the following: it is used as
	 * JGTimer tick increment, animation increment, JGObject expiry increment, and
	 * is used as multiplier for object x/yspeed, and for the default margins of
	 * is...Aligned and snapToGrid.
	 */
	public void setGameSpeed(double speed);

	/**
	 * Get game speed variable. This can be used if you have other stuff in your
	 * game that is affected by game speed, besides the standard game speed
	 * adaptation done by the engine.
	 */
	public double getGameSpeed();

	/**
	 * Configure image rendering. alpha_thresh is used to determine how a
	 * translucent image is converted to a bitmask image. Alpha values below the
	 * threshold are set to 0, the others to 255. render_bg_col is used to render
	 * transparency for scaled images; it is the background colour that
	 * interpolations between transparent and non-transparent pixels are rendered
	 * to. Currently, this has an effect in Jdk1.2 only. The default render_bg_col
	 * is null, meaning the global background colour is used.
	 * 
	...
	/**
	 * Set margin used for testing if object should expire or suspend when off-view
	 * or off-playfield. Default is 16,16.
	 */
	...
	/**
	 * Set mouse cursor, platform dependent. null (platform-independent) means hide
	 * cursor.
	 */
	 ...
	 * doFrame&lt;state&gt; and paintFrame&lt;state&gt; will be called in addition
	 * to doFrame() and paintFrame(). Before the next frame, start&lt;state&gt; is
	 * called once. Note that setGameState may actually set a state that's already
	 * set, in which case start&lt;state&gt; is not called. Also, if the
	 * setGameState is superseded by another setGameState within the same frame, the
	 * first setGameState is ignored.
	 */
	public void setGameState(String state);

	/**
	 * Add the given state to the game's existing state on the next frame. The
	 * methods doFrame&lt;state&gt; and paintFrame&lt;state&gt; will be called <i>in
	 * addition to</i> the methods of any states already set. Before the next frame,
	 * start&lt;state&gt; is called once. Note that addGameState may actually set a
	 * state that's already set, in which case start&lt;state&gt; is not called.
	 */
	...
	/**
	 * Set the game's main state to none, on the next frame. Only doFrame() and
	 * paintFrame() will be called each frame.
	 */
	...
	/**
	 * Is called every frame. Override to define frame action. Default is do
	 * nothing.
	 */
	public void doFrame();

	/**
	 * Is called when the engine's default screen painting is finished, and custom
	 * painting actions may be carried out. Can be used to display status
	 * information or special graphics. Default is do nothing.
	 */
	public void paintFrame();

	/**
	 * Get the graphics context for drawing on the buffer during a paintFrame().
	 * Returns null when not inside paintFrame.
	 */
	...
	/**
	 * Set blend mode, for platforms that support blending. Blend functions
	 * supported are based on the alpha of the object being drawn (the drawing
	 * source). Source and destination alpha multiplier are specified separately.
	 * These are one of: always 1 (encoded as 0), alpha (encoded as 1), and 1-alpha
	 * (encoded as -1). Default is (1,-1).
	 * 
	...
	/**
	 * DrawLine combined with thickness/colour setting. The line is drawn relative
	 * to the playfield coordinates.
	 */
	public void drawLine(double x1, double y1, double x2, double y2, double thickness, JGColor color);

	/**
	 * Draw a line with current thickness and colour. The line is drawn relative to
	 * the playfield coordinates.
	 */
	...
	/**
	 * Draw convex polygon. For filled polygons, it is possible to draw a colour
	 * gradient, namely a "fan" of colours (specified by col), spreading from the
	 * first point to each successive point in the polygon. For line polygons, a
	 * gradient is drawn between successive line segments. On non-OpenGL platforms,
	 * the fill gradient is drawn as a fan of plain colours, with colour 0 and 1
	 * ignored, the line gradient as plain coloured lines.
	 *
	...
	/**
	 * Draw rectangle in default colour and thickness. Coordinates are relative to
	 * playfield.
	 * 
	 * @param centered indicates (x,y) is center instead of topleft.
	 */
	...
	/**
	 * Draw shaded rectangle. On non-opengl platforms, rectangle is drawn in default
	 * colour.
	 * 
	...
	/**
	 * Draw shaded or patterned rectangle. On non-opengl platforms, rectangle is
	 * drawn in default colour.
	 * 
	 * @param shadecol colors topleft,topright,botright,botleft corners
	 */
	public void drawRect(double x, double y, double width, double height, boolean filled, boolean centered,
	...
	/**
	 * Draw oval with default thickness and colour. Coordinates are relative to
	 * playfield.
	 * 
	 * @param centered indicates (x,y) is center instead of topleft.
	 */
	...
	/**
	 * Extended version of drawImage for OpenGL or Android. On platforms without
	 * support for accelerated blending, rotation, scaling, this call is equivalent
	 * to drawImage(x,y,imgname,pf_relative).
	 *
	...
	/**
	 * Extended version of drawImage for OpenGL or Android, new version. On
	 * platforms without support for accelerated blending, rotation, scaling, this
	 * call is equivalent to drawImage(x,y,imgname,pf_relative).
	 *
	 * rotation and scaling are centered around the image center.
	 *
	 * @param blend_col colour to blend with image, null=(alpha,alpha,alpha)
	...
	/**
	 * Draws string so that (x,y) is the topleft coordinate (align=-1), the top
	 * middle coordinate (align=0), or the top right coordinate (align=1). Use given
	 * font and colour; filling in null for either means ignore. Unlike the other
	 * draw functions, for strings, coordinates are relative to view by default. An
	 * outline is drawn around the text when defined by setTextOutline.
	 * 
	...
	/**
	 * Draws string so that (x,y) is the topleft coordinate (align=-1), the top
	 * middle coordinate (align=0), or the top right coordinate (align=1). Use
	 * current font and colour. Unlike the other draw functions, for strings,
	 * coordinates are relative to view by default. An outline is drawn around the
	 * text when defined by setTextOutline.
	 * 
	 * @param align       text alignment, -1=left, 0=center, 1=right
	 * @param pf_relative
	 * @param font
	 ...
	 * Draws string so that (x,y) is the topleft coordinate (align=-1), the top
	 * middle coordinate (align=0), or the top right coordinate (align=1). Use
	 * current font and colour. An outline is drawn around the text when defined by
	 * setTextOutline.
	 * 
	 * @param align       text alignment, -1=left, 0=center, 1=right
	 * @param pf_relative coordinates are relative to playfield, otherwise view
	...
	/**
	 * Draws a single line of text using an image map as font; text alignment is
	 * same as drawString. Typically, an image font only defines the ASCII character
	 * range 32-96. In this case, set char_offset to 32, and use only the uppercase
	 * letters. Coordinates are relative to view.
	 * 
	 * @param align       text alignment, -1=left, 0=center, 1=right
	...
	/**
	 * Get current mouse position in logical coordinates, inverse projected through
	 * last set view zoom/rotate setting.
	 */
	...
	/**
	 * Returns true if multitouch supported (currently only on Android devices).
	 */
	public boolean hasMultitouch();

	/**
	 * get IDs of touch pointers for multitouch handling. You can use getMouse...
	 * instead, if multitouch is not required.
	 * <p>
	 * Touch IDs are globally unique numbers identifying multitouch finger
	 * trajectories (touch-drag-release). The IDs returned by this function can be
	 * used to get all the info you need for tracking finger touches and releases.
	 * The list is refreshed every frame. Use getTouchX/Y to get info about the
	 * positions.
	 * 
	 * @return list of IDs, or null when multitouch not supported.
	 */
	public int[] touchGetIDs();

	/**
	 * get X position of given touch pointer. Returns 0 when ID does not exist.
	 */
	public double touchGetX(int id);

	/**
	 * get Y position of given touch pointer. Returns 0 when ID does not exist.
	 */
	public double touchGetY(int id);

	/**
	 * Check if touch ID is still active. This is equivalent to calling touchGetIDs
	 * and searching the result for the given ID.
	 * 
	 * @return true if the given ID is in touchGetIDs
	 */
	...
	/**
	 * Obtain key code from printable string describing the key, the inverse of
	 * getKeyDesc. The string is trimmed and lowercased.
	 */
	public int getKeyCode(String keydesc);

	/**
	 * returns true if device has accelerometer... Hacked up by Mr. Hapke
	 */
	...
	/** get gyroscope vector X coordinate */
	public double getGyroX();

	/** get gyroscope vector Y coordinate */
	public double getGyroY();

	/** get gyroscope vector Z coordinate [twist] (positive = ???) */
	public double getGyroZ();

	/** get gyroscope vector X angle */
	public double getGyroAngleX();

	/** get gyroscope vector Y angle */
	public double getGyroAngleY();

	/** get gyroscope vector Z angle [twist] (positive = ???) */
	public double getGyroAngleZ();

	/** get accel/gyroscope Left-Right angle */
	public double getPitch();

	/** get accel/gyroscope Front-Back angle */
	...
	/**
	 * Get animation definition, don't call directly. This is used by JGObjects to
	 * get animations.
	 */
	...
	/**
	 * Returns path to writable location for a file with the given name. Basically
	 * it uses [user.home] / .jgame / [filename], with "/" being the system path
	 * separator. In case [user.home] / .jgame does not exist, it is created. In
	 * case .jgame is not a directory, null is returned. In case the file does not
	 * exist yet, an empty file is created.
	 * 
	...
	/**
	 * Generates discrete random number between min and max inclusive, with steps of
	 * interval. Epsilon is added to max to ensure there are no rounding error
	 * problems with the interval. So, random(0.0, 4.2, 2.1) generates either 0.0,
	 * 2.1, or 4.2 with uniform probabilities. If max is halfway between interval
	 * steps, max is treated as exclusive. So, random(0.0,5.0,2.1) generates 0.0,
	 * 2.1, 4.2 with uniform probabilities. If you need integer ranges, be sure to
	 * use the integer version to avoid rounding problems.
	 */
	public double random(double min, double max, double interval);

	/**
	 * Generates discrete random number between min and max inclusive, with steps of
	 * interval, integer version. If max is halfway between two interval steps, it
	 ...
	 * implementation simply uses Math.atan2, for MIDP a short and fast Math.atan2
	 * replacement is used, with average numerical error less than 0.001 radians,
	 * maximum error 0.005 radians.
	 */
	...
	/**
	 * Get pixel coordinate corresponding to the top left of the tile at the given
	 * index
	 */
	...
	/**
	 * Snap to grid, double version. Epsilon is added to the gridsnap value, so that
	 * isXAligned(x,margin) always implies that snapToGridX(x,margin) will snap.
	 * 
	...
	/**
	 * Snap to grid, double version. Epsilon is added to the gridsnap value, so that
	 * isYAligned(y,margin) always implies that snapToGridY(y,margin) will snap.
	 * 
	 ...
	 * function only handles integers so it should not be used to snap an object
	 * position.
	 */
	public void snapToGrid(JGPoint p, int gridsnapx, int gridsnapy);

	/**
	 * Returns true if x falls within margin of the tile snap grid. Epsilon is added
	 * to the margin, so that isXAligned(1.0000, 1.0000) always returns true.
	 */
	public boolean isXAligned(double x, double margin);

	/**
	 * Returns true if y falls within margin of the tile snap grid. Epsilon is added
	 * to the margin, so that isYAligned(1.0000, 1.0000) always returns true.
	 */
	...
	/**
	 * Calculates length of the shortest path between x1 and x2, with x1, x2 being
	 * playfield coordinates, taking into account the wrap setting.
	 */
	public double getXDist(double x1, double x2);

	/**
	 * Calculates length of the shortest path between y1 and y2, with y1, y2 being
	 * playfield coordinates, taking into account the wrap setting.
	 */
	...
	/**
	 * Disable audio, stop all currently playing audio. Audio commands will be
	 * ignored, except that audio loops (music, ambient sounds) are remembered and
	...
	/**
	 * Adds title to be displayed above subsequent items. Default title of initial
	 * items is "Preferences". Call this before defining any items to override the
	 * default title.
	 */
	public void optsAddTitle(String title);

	/**
	 * adds (int or float) number that can be configured with a slider. Type is
	 * double if decimals!=0, int otherwise.
	 * 
	...
	/**
	 * Adds boolean that can be configured with a checkbox. Actual type is int (0 or
	 * 1)
	 */
	public void optsAddBoolean(String varname, String title, String desc, boolean initial);

	/**
	 * Adds enum that can be configured with radio buttons. Actual type is int, 0
	 * for first item, 1 for second item, etc.
	 */
	public void optsAddEnum(String varname, String title, String desc, String[] values, int initial);

	/**
	 * Adds "key" option that can be configured by selecting a key or button on the
	 * device. Actual type is int.
	 */
