...
public class EngineLogic {
	...
	/**
	 * make_bitmask indicates what to do with transparent images. true = make
	 * bitmask false=make translucent
	 */
	...
	/**
	 * Note: objects lock is used to synchronise object updating between repaint
	 * thread and game thread. The synchronize functions are found in
	...
	public JGImage getImage(String imgname) {
		...
		if (img == null) {
			...
			if (render_bg_col == null)
				render_bg_col = bg_color;
			img = img.toDisplayCompatible(alpha_thresh, render_bg_col, true, make_bitmask);
		...
		try {
			...
			while ((line = readline(in)) != null) {
				...
				if (i == 8) {
					...
					defineImage(eng, fields[0], fields[1], Integer.parseInt(fields[2]), fields[3], fields[4],
							Integer.parseInt(fields[5]), Integer.parseInt(fields[6]), Integer.parseInt(fields[7]),
							Integer.parseInt(fields[8]));
				} else if (i == 5) {
					defineImage(eng, fields[0], fields[1], Integer.parseInt(fields[2]), fields[3], fields[4], -1, -1,
							-1, -1);
				} else if (i == 10) {
					defineImage(fields[0], fields[1], Integer.parseInt(fields[2]),
							getSubImage(fields[3], Integer.parseInt(fields[4])), fields[5], Integer.parseInt(fields[6]),
							Integer.parseInt(fields[7]), Integer.parseInt(fields[8]), Integer.parseInt(fields[9]));
				} else if (i == 6) {
					defineImage(fields[0], fields[1], Integer.parseInt(fields[2]),
							getSubImage(fields[3], Integer.parseInt(fields[4])), fields[5], -1, -1, -1, -1);
				} else if (i == 3) {
					defineAnimation(fields[0], splitList(fields[1]), Double.parseDouble(fields[2]));
				} else if (i == 4) {
					defineAnimation(fields[0], splitList(fields[1]), Double.parseDouble(fields[2]),
	...
	public void defineImage(Object pkg_obj, String name, String tilename, int collisionid, String imgfile,
		...
		defineImage(name, tilename, collisionid, img, img_op, top, left, width, height);
	}
	...
	public void defineImageFromData(String name, String tilename, int collisionid, int imgwidth, int imgheight,
		...
		JGImage img = imageutil.createImageFromData(imgwidth, imgheight, imgdata, imgdataofs, linewidth);
		defineImage(name, tilename, collisionid, img, img_op, top, left, width, height);
	}
	...
	public void defineImageRotated(Object pkg_obj, String name, String tilename, int collisionid, String srcname,
			double angle) {
	...
	public void defineImageMap(Object pkg_obj, String mapname, String imgfile, int xofs, int yofs, int tilex, int tiley,
			int skipx, int skipy) {
		imgfile = getAbsolutePath(pkg_obj, imgfile);
		imagemaps.put(mapname, new ImageMap(imageutil, imgfile, xofs, yofs, tilex, tiley, skipx, skipy));
	}
	...
	public void defineImage(Object pkg_obj, String imgname, String tilename, int collisionid, String imgfile,
			String img_op) {
		defineImage(pkg_obj, imgname, tilename, collisionid, imgfile, img_op, -1, -1, -1, -1);
	}

	public void defineImage(String imgname, String tilename, int collisionid, String imgmap, int mapidx, String img_op,
			int top, int left, int width, int height) {
		defineImage(imgname, tilename, collisionid, getSubImage(imgmap, mapidx), img_op, top, left, width, height);
	}

	public void defineImage(String imgname, String tilename, int collisionid, String imgmap, int mapidx,
			String img_op) {
		defineImage(imgname, tilename, collisionid, getSubImage(imgmap, mapidx), img_op, 0, 0, -1, -1);
	}
	...
	void markRemoveObject(String index) {
		...
		if (idx < 0)
			return;
		obj_to_remove.put(index, objects.values[idx]);
	}
	...
	public void moveObjects(JGEngineInterface eng, String prefix, int cidmask) {
		...
		for (int i = firstidx; i < lastidx; i++) {
			...
			if (o.is_suspended) {
				if (o.resume_in_view && o.isInView(offscreen_margin_x, offscreen_margin_y))
					o.resume();
			} else {
				if (o.expiry == JGObject.suspend_off_view || o.expiry == JGObject.suspend_off_view_expire_off_pf) {
			...
			if (!o.is_suspended) {
				...
				if (expiry >= 0) {
					...
					if (expiry == JGObject.expire_off_pf || expiry == JGObject.suspend_off_view_expire_off_pf) {
						if (!o.isOnPF(offscreen_margin_x, offscreen_margin_y))
							o.remove();
					}
					if (expiry == JGObject.expire_off_view && !o.isInView(offscreen_margin_x, offscreen_margin_y))
						o.remove();
				}
			}
		}
	...
	public void checkBGCollision(JGEngineInterface eng, int tilecid, int objcid) {
		...
		for (int i = 0; i < srcsize; i++) {
			...
			if ((cid & tilecid) != 0) {
				try {
					o.hit_bg(cid);
					o.hit_bg(cid, tiler.getX(), tiler.getY(), tiler.getWidth(), tiler.getHeight());
					// XXX this might be slow, check its performance
					for (int y = 0; y < tiler.getHeight(); y++) {
						for (int x = 0; x < tiler.getWidth(); x++) {
							int thiscid = getTileCid(tiler.getX() + x, tiler.getY() + y);
							if ((thiscid & tilecid) != 0)
								o.hit_bg(thiscid, tiler.getX() + x, tiler.getY() + y);
						}
					}
				} catch (JGameError ex) {
					eng.exitEngine(eng.dbgExceptionToString(ex));
	...
	public void repaintBG(JGEngineInterface eng) {
		...
		for (int x = 0; x < viewnrtilesx + 3; x++) {
			int xi = x + tilexofs;
			// check if out of playfield bounds, only needed if not wrapping.
			if (!pf_wrapx && (xi < 0 || xi >= nrtilesx))
				continue;
			int xi_mod = moduloFloor(xi, viewnrtilesx + 3);
			int xi_modpf = xi;
			if (pf_wrapx)
				xi_modpf = moduloFloor(xi, nrtilesx);
			// determine loop bounds for tight loop
			int ylower = tileyofs;
			int yupper = tileyofs + viewnrtilesy + 3;
			if (!pf_wrapy) {
				...
				if (ylower > nrtilesy)
					continue;
				if (yupper > nrtilesy)
					yupper = nrtilesy; // exclusive bound
			}
			...
			for (int yi = ylower; yi < yupper; yi++) {
				...
				int yi_mod = (yi >= 0) ? yi % divfactor : divfactor - 1 - ((-1 - yi) % divfactor);
				if (bg_defined[xi_mod][yi_mod])
					continue;
				int yi_modpf = yi;
				if (pf_wrapy)
					yi_modpf = moduloFloor(yi, nrtilesy);
				...
				if (x == 0 || y == 0 || x == viewnrtilesx + 2 || y == viewnrtilesy + 2) {
					...
					if (nrtilesdrawn > maxpredrawtiles)
						continue;
					// nrpredrawtilesdrawn++;
				}
	...
	public void setBGImage(String bgimg, int depth, boolean wrapx, boolean wrapy, boolean extended_mode) {
		if (extended_mode && !bg_extended_mode) {
			invalidateBGTiles();
			bg_extended_mode = true;
		}
		while (bg_images.size() < depth + 1)
			bg_images.addElement(null);
		if (bgimg != null) {
			if (!images.containsKey(bgimg) && !images_orig.containsKey(bgimg))
			...
			while (bg_images.size() > 1 && bg_images.elementAt(bg_images.size() - 1) == null) {
	...
	public void setTile(int x, int y, String tilestr) {
		...
		if (!pf_wrapx)
			if (x_mod < tilexofs || x_mod >= tilexofs + viewnrtilesx + 3)
				return;
		if (!pf_wrapy)
			if (y_mod < tileyofs || y_mod >= tileyofs + viewnrtilesy + 3)
				return;
		// find offset in bg
		// int x_mod_bg = moduloFloor(x,viewnrtilesx+3);
		// int y_mod_bg = moduloFloor(y,viewnrtilesy+3);
		// version with moduloFloor inlined
		int x_mod_bg = (x >= 0) ? x % (viewnrtilesx + 3) : viewnrtilesx + 2 - ((-1 - x) % (viewnrtilesx + 3));
		int y_mod_bg = (y >= 0) ? y % (viewnrtilesy + 3) : viewnrtilesy + 2 - ((-1 - y) % (viewnrtilesy + 3));
		// in case we wrap and bg is larger than pf, ensure we are within
		// 0 < x_mod_bg < nrtilesx
		if (x_mod_bg >= nrtilesx)
			x_mod_bg -= nrtilesx;
		if (y_mod_bg >= nrtilesy)
			y_mod_bg -= nrtilesy;
		// invalidate tile
	...
	public int countTiles(int tilecidmask) {
		...
		for (int x = 0; x < nrtilesx; x++) {
			for (int y = 0; y < nrtilesy; y++) {
				if ((tilecidmap[x][y] & tilecidmask) != 0)
					count++;
			}
		}
		return count;
	...
	public int getTileCid(JGRectangle tiler) {
		int cid = 0;
		for (int x = tiler.getX(); x < tiler.getX() + tiler.getWidth(); x++)
			for (int y = tiler.getY(); y < tiler.getY() + tiler.getHeight(); y++)
				cid |= getTileCid(x, y);
		return cid;
	}
	...
	private int tilestrToCid(String tilestr) {
		if (tilestr == null || tilestr.length() == 0)
			return 0;
		Integer tileid = (Integer) images_tilecid.get(new Integer(tileStrToID(tilestr)));
		if (tileid == null) {
	...
	private int tileintToCid(int tileint) {
		if (tileint == 0)
			return 0;
		Integer tileid = (Integer) images_tilecid.get(new Integer(tileint));
		if (tileid == null) {
	...
	public String tileIDToStr(int tileid) {
		...
		if (tileid >= 0x100)
			tilestr.append((char) ((tileid / 0x100) & 255));
		if (tileid >= 0x10000)
			tilestr.append((char) ((tileid / 0x10000) & 255));
		if (tileid >= 0x1000000)
			tilestr.append((char) ((tileid / 0x1000000) & 255));
	...
	void convertToTiles(JGRectangle dest, JGRectangle r) {
		if (dest.getX() >= 0) {
			dest.setX(dest.getX() / tilex);
			dest.setWidth(1 - dest.getX() + (r.getX() + r.getWidth() - 1) / tilex);
		} else {
			dest.setX((dest.getX() - tilex + 1) / tilex);
			dest.setWidth(1 - dest.getX() + (r.getX() + r.getWidth() - 1 - tilex + 1) / tilex);
		}
		if (dest.getY() >= 0) {
			dest.setY(dest.getY() / tiley);
			dest.setHeight(1 - dest.getY() + (r.getY() + r.getHeight() - 1) / tiley);
		} else {
			dest.setY((dest.getY() - tiley + 1) / tiley);
			dest.setHeight(1 - dest.getY() + (r.getY() + r.getHeight() - 1 - tilex + 1) / tiley);
		}
	}
	...
	public void setTiles(int xofs, int yofs, String[] tilemap) {
		for (int y = 0; y < tilemap.length; y++) {
			for (int x = 0; x < tilemap[y].length(); x++) {
				setTile(x + xofs, y + yofs, new String(tilemap[y].substring(x, x + 1)));
			}
		}
	}
	...
	public void drawImageString(JGEngineInterface eng, String string, double x, double y, int align, String imgmap,
		...
		for (int i = 0; i < string.length(); i++) {
			...
			if (!existsImage(lettername)) {
				defineImage(lettername, "FONT", 0, getSubImage(imgmap, imgnr), "-", 0, 0, 0, 0);
			}
	...
	/**
	 * Div which always rounds downwards, also for negative numbers. It is the
	 * counterpart of moduloFloor.
	 */
	...
	/**
	 * Modulo according to a regular modulo space, resulting in a value that is
	 * always greater than 0, float version. It is the counterpart of divFloor.
	 */
	...
	public int scaleXPos(double x, boolean pf_relative) {
		if (!pf_relative) {
			return (int) Math.floor(x_scale_fac * x);
		} else {
			if (pf_wrapx)
				x = moduloXPos(x);
			return (int) Math.floor(x_scale_fac * x) - xofs_scaled;
		}
	}

	public int scaleYPos(double y, boolean pf_relative) {
		if (!pf_relative) {
			return (int) Math.floor(y_scale_fac * y);
		} else {
			if (pf_wrapy)
				y = moduloYPos(y);
			return (int) Math.floor(y_scale_fac * y) - yofs_scaled;
		}
	}
	...
	public JGRectangle scalePos(JGRectangle r, boolean pf_relative) {
		if (r == null)
			return null;
		return scalePos(r.getX(), r.getY(), r.getWidth(), r.getHeight(), pf_relative);
	}
	...
	public void initPF() {
		...
		double aspectratio = (scaledtilex / (double) scaledtiley) / (tilex / (double) tiley);
	...
	public JGPoint getTileIndex(double x, double y) {
		return new JGPoint((int) Math.floor(x / tilex), (int) Math.floor(y / tiley));
	}
	...
	public double snapToGridX(double x, double gridsnapx) {
		if (gridsnapx <= 0.0)
			return x;
		int xaligned = tilex * (int) Math.floor(((x + tilex / 2.0) / tilex));
		double gridofsx = Math.abs(x - xaligned);
		if (gridofsx <= gridsnapx + 0.0002)
			return xaligned;
		return x;
	}
	...
	public double snapToGridY(double y, double gridsnapy) {
		if (gridsnapy <= 0.0)
			return y;
		int yaligned = tiley * (int) Math.floor(((y + tiley / 2.0) / tiley));
		double gridofsy = Math.abs(y - yaligned);
		if (gridofsy <= gridsnapy + 0.0002)
			return yaligned;
		return y;
	}
	...
	public void snapToGrid(JGPoint p, int gridsnapx, int gridsnapy) {
		if (gridsnapx == 0 && gridsnapy == 0)
			return;
		int xaligned = tilex * (int) Math.floor(((p.x + tilex / 2.0) / tilex));
		int yaligned = tiley * (int) Math.floor(((p.y + tiley / 2.0) / tiley));
	...
	public boolean isXAligned(double x, double margin) {
		if (margin < 0)
			margin = 0.0;
		int xaligned = tilex * (((int) x + tilex / 2) / tilex);
	...
	public boolean isYAligned(double y, double margin) {
		if (margin < 0)
			margin = 0.0;
		int yaligned = tiley * (((int) y + tiley / 2) / tiley);
	...
	public double getXDist(double x1, double x2) {
		if (pf_wrapx) {
			...
			return Math.min(Math.abs(x1mod - x2mod), Math.abs(x1mod + pfwidth - x2mod));
		} else {
	...
	public double getYDist(double y1, double y2) {
		if (pf_wrapy) {
			...
			return Math.min(Math.abs(y1mod - y2mod), Math.abs(y1mod + pfheight - y2mod));
		} else {
