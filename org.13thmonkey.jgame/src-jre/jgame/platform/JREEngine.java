 ...
 * misc.
 */
class JREEngine implements MouseListener, MouseMotionListener, FocusListener, KeyListener, WindowListener {
	...
	public void keyPressed(KeyEvent e) {
		...
		/* shift escape = exit */
		if (e.isShiftDown() && e.getKeyCode() == KeyEvent.VK_ESCAPE && !eng.isApplet()) {
			System.exit(0);
		}
	...
	public JGPoint getMousePos() {
		return new JGPoint(mousepos.x, mousepos.y);
	}
	...
	public static String getKeyDescStatic(int key) {
		...
		if (key == 0)
			return "(none)";
		if (key == JGEngineInterface.KeyEnter)
			return "enter";
		if (key == JGEngineInterface.KeyEsc)
			return "escape";
		if (key == JGEngineInterface.KeyUp)
			return "cursor up";
		if (key == JGEngineInterface.KeyDown)
			return "cursor down";
		if (key == JGEngineInterface.KeyLeft)
			return "cursor left";
		if (key == JGEngineInterface.KeyRight)
			return "cursor right";
		if (key == JGEngineInterface.KeyShift)
			return "shift";
		if (key == JGEngineInterface.KeyAlt)
			return "alt";
		if (key == JGEngineInterface.KeyCtrl)
			return "control";
		if (key == JGEngineInterface.KeyMouse1)
			return "left mouse button";
		if (key == JGEngineInterface.KeyMouse2)
			return "middle mouse button";
		if (key == JGEngineInterface.KeyMouse3)
	...
	/**
	 * Note: this assumes that primitive type wrappers: Integer, Char, Boolean,
	 * Double, Float are always primitive types
	 */
	static Method getMethod(Class<? extends Object> cls, String name, Object[] args) {
		@SuppressWarnings("unchecked")
		Class<? extends Object>[] args_cls = (Class<? extends Object>[]) new Class[args.length];
	...
	/**
	 * Try to execute given method on given object. Handles invocation target
	 * exceptions.
	 * 
	...
	static boolean existsMethod(Class<? extends Object> cls, String name, Object[] args) {
		return getMethod(cls, name, args) != null;
	}

	static boolean existsMethod(Class<? extends Object> cls, String name, Class<? extends Object>[] args) {
		try {
	...
	void createWindow(Component appwin, boolean add_decoration) {
		...
		if (existsMethod(Frame.class, "setUndecorated", new Class[] { Boolean.TYPE })) {
			/* this is the jdk1.4+ way to do it */
			my_win = new Frame();
			tryMethod(my_win, "setUndecorated", new Object[] { new Boolean(!add_decoration) });
		} else {
			/*
			 * jdk1.2: adding a window to a frame like this results in a window without
			 * decoration.
			 */
			if (!add_decoration) {
				my_frame = new Frame();
				...
				/*
				 * in jdk1.4, we need to call the following two methods to ensure we can get the
				 * focus. However, jdk1.2 doesn't have the setFocusableWindowState method
				 * however.
				 */
				// my_win.setFocusableWindowState(true);
				tryMethod(my_win, "setFocusableWindowState", new Object[] { new Boolean(true) });
	...
	void setWindowSize(boolean add_decoration) {
		if (!add_decoration) {
			...
			my_win.setSize(el.winwidth + insets.left + insets.right, el.winheight + insets.top + insets.bottom);
		}
	...
	Hashtable<String, String> islooping = new Hashtable<String, String>();

	/**
	 * clipd -} "yes". Sample has already been triggered on an unnamed channel
	 * during this frame.
	 */
	Hashtable<String, String> clipstriggered = new Hashtable<String, String>();
	...
	void audioNewFrame() {
		clipstriggered = new Hashtable<String, String>();
	}
	...
	Hashtable<String, Object> localstore = new Hashtable<String, Object>();
	...
	private void storeWrite() {
		try {
			if (localstorefile == null)
				return;
			PrintWriter writer = new PrintWriter(new FileWriter(new File(localstorefile)));
			for (Enumeration<String> e = localstore.keys(); e.hasMoreElements();) {
